...
def st_write_code_from_file(file_name, expanded=True, *args, **kwargs) -> None:
    with st.expander(label=f" {file_name}", expanded=expanded):
        with open(f"/home/antonia/code/Unlocking-Information/{file_name}", 
                  "r") as f:
...
def get_image_paths(folder_name=""):
    ...
    return (path_len_of_height_plt, path_len_of_sex_plt, path_col_shapes_plt,
            path_age_height_plt, path_mng_severity_plt)
