...
@st.cache_data
def get_eval_utils(synth_data_path="synthetic_rpad.csv",
...
with st.sidebar:
    ...
    genre = st.radio(
        "Synthetic data",
        ('default', 'with metadata', 'with constraints', 'with tuning',
         'best setting'))
    ...
    if genre == 'default':
        ...
        (path_len_of_height_plt,
         ...
         path_col_shapes_plt,
         path_age_height_plt,
         path_mng_severity_plt) = get_image_paths('default')
    if genre == 'with metadata':
        ...
        (path_len_of_height_plt,
         ...
         path_col_shapes_plt,
         path_age_height_plt,
         path_mng_severity_plt) = get_image_paths('metadata')
    if genre == 'with constraints':
        ...
        (path_len_of_height_plt,
         ...
         path_col_shapes_plt,
         path_age_height_plt,
         path_mng_severity_plt) = get_image_paths('constraints')
    if genre == 'with tuning':
        ...
        (path_len_of_height_plt,
         ...
         path_col_shapes_plt,
         path_age_height_plt,
         path_mng_severity_plt) = get_image_paths('tuning')
    if genre == 'best setting':
        ...
        (path_len_of_height_plt,
         ...
         path_col_shapes_plt,
         path_age_height_plt,
         path_mng_severity_plt) = get_image_paths('best_setting')
...
with tab3:
    ...
    with st.expander(label="""SDV Quality Report includes statistical tests
    ...
    with st.expander(label="Column Shapes Details",
                     expanded=False):
        st.write(report_q.get_details(property_name='Column Shapes'))

    with st.expander(label="Column Pair Trends",
                     expanded=False):
...
with tab4:
    ...
    with st.expander(label="Numerical variable plot",
                     expanded=False):
        st.image(path_len_of_height_plt)

    with st.expander(label="Categorical variable plot",
                     expanded=False):
        st.image(path_len_of_sex_plt)

    st.markdown("### Visual inspection of column pairs:")

    with st.expander(label="Plotting 'Weight' and 'Height'",
                     expanded=False):
        st.image(path_age_height_plt)

    with st.expander(label="Plotting 'Management' and 'Severity'",
                     expanded=False):
