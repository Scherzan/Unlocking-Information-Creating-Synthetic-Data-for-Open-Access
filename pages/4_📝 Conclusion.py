...
tab1, tab2, tab3, tab4 = st.tabs(["Our project learnings.💡", "So far so good. ✅", "Where to go next? 🧭", "Thank you. 💛"])
...
with tab1:
    ...
    no_data_access = st.checkbox("No access to data due to privacy regulations")
    if no_data_access:
       text_2 = """
    ...
    if no_hardware:
       text_3 = """
       An absolute must. Make sure to know who the final users will be and what they want to use the data for.
       """
       st.markdown(f'<p style="color:#1f449c">{text_3}</p>', unsafe_allow_html=True)
...
with tab2:
    ...
    no_ds_team = st.checkbox("We heard about synthetic data and how to generate it.")
    if no_ds_team:
       text = """
       Yep, quite easy. pip install SDV - load data - feed data into model - and use .sample method
       """
       st.markdown(f'<p style="color:#1f449c">{text}</p>', unsafe_allow_html=True)
